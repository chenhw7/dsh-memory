...
# Storage rubric v2 — scoring a stored memory entry against planted facts
...
facts"). Afterwards the plugin's automatic extraction wrote or updated
entries in the memory store. You score exactly ONE stored entry against the
planted facts, using only the anchors in this document, and you answer with
strict JSON.
...
Scores under this v2 scale are not comparable with v1 scores (the judged
population grew to include in-place updates, and dimension 2 gained pinned
ground truth); the report stamps the rubric version for that reason.

## Inputs (one JSON object per call)

- `scenarioId` — for the report only; never scored.
- `plantedFacts` — array of `{ "id", "statement", "expectedScope"?, "expectedCategory"? }`:
  every fact the scenario planted. The runner materializes `statement` from
  the corpus by one fixed rule per scenario kind (one home per fact id):
  - plant scenario — the `turns[].user` message, word for word, of the FIRST
    turn whose `planted` list carries that fact id; EXCEPT when the
    scenario's `plantFacts` table carries a `factText` for the id — that
    normalized clean excerpt is the statement instead (a fact buried
    mid-way through a long noisy turn must not make the whole turn the
    ground truth);
  - seed scenario — the `seedEntries[].content` of the entry with that id.
  These statements are the ONLY ground truth. `expectedScope` /
  `expectedCategory`, when present on a fact, are the scenario's PINNED
  scope/category ground truth for Dimension 2; when absent, infer from the
  routing rules below.
- `entry` — the stored entry under review:
  `{ "id", "scope", "category", "summary", "content" }` (`category` and
  `summary` may be absent), plus `updated: true` when the entry PRE-EXISTED
  the session and was changed in it (the medium-diff case below). A new
  write never carries the flag.
- `storeBefore` — entries that existed before the session (may be empty). An
  `updated` entry's pre-session version is here under the SAME id.
- `siblings` — the session's other written-or-updated entries (may be empty).

The conversation transcript is not provided. Do not invent facts beyond the
statements; do not assume unstated context.

## Medium-diff basis (new in v2)

The judged population is every entry the session WROTE or UPDATED:

- written — its id is not in `storeBefore`;
- updated — its id is in `storeBefore` and at least one of content / scope /
  category / summary changed in-session (timestamps and `projectName`
  bookkeeping do not count).

An unchanged pre-existing entry is not judged. An update keeps its id, so an
in-place merge reaches the judge as an `updated: true` entry — the blind
spot of the v1 written-only basis. When the entry is `updated: true`, judge
Dimension 4 against the pre-session version under the same id in
`storeBefore`.
...
or asserts a conflicting value for that fact's subject: a wrong number, an
inverted direction, a competing mechanism on the fact's own topic is still
ABOUT that fact. Traceability follows the subject, not the truth: a
contradicting entry keeps `plantedId` set to that fact's id, and the
contradiction is scored where it belongs — Dimension 1, tier 0. Set
`plantedId` to the traced fact's id; otherwise `null`. `null` is reserved for
entries whose subject no planted fact covers: fabricated details nobody
stated, records of ordinary repository structure (file trees, APIs, diffs,
commit history), and restatements of the situation that carry no planted
fact. The harness counts `plantedId: null` entries as extraction
hallucinations in the scenario-level precision metric defined below — but
you still score all four dimensions honestly.
...
Normalization in both directions is free: when the statement was voiced
noisily (homophone typos, voice-input phrasing, a mid-sentence
self-correction whose final value stands, fillers), cleaning it into fluent
text is NOT fabrication, and keeping the noisy wording verbatim is NOT a
loss. Orthography is not scored; meaning is.
- ✓（计 2）事实原句带同音错字「在 CI 上必须跑全量测试」→ 条目「CI 上必须跑全量测试」：错字归一化，断言与限定词不变。
- ✓（计 2）条目逐字保留了原句的口头语与「不对，是…」式自我纠正的终值：语义完整，不算丢失。
...
Score the entry's `scope` and `category` against the ground truth for the
traced planted fact: its PINNED `expectedScope` / `expectedCategory` when the
corpus carries them, otherwise these fixed routing rules (an untraced entry
scores by what its content claims to be).
...
- **2 — scope correct, and category correct or legitimately absent.**
  ...
  - ✓（计 2）埋点钉了 `expectedScope: global` 与 `expectedCategory: convention`，条目存 `global` / `convention`：与钉定值一致。
  - ✗（不计 2，降至 1）埋点钉了 `expectedScope: user`，条目存成 `project`：作用域偏离钉定值。
...
A value the entry contradicts is not a missing token: a wrong number on the
fact's own slot keeps the entry on the fact's retrieval path — the ask still
surfaces it — and the wrongness belongs to Dimension 1. A token counts as
lost only when the entry replaced it with vagueness (abstraction) or with
another topic, not when it filled the same slot with a conflicting value.

- **2 — stable under rewording.** All distinctive tokens are present —
  including the slots whose values the entry states wrongly (the conflict is
  fidelity's finding; the entry is still found by the fact's ask).
  - ✓（计 2）事实「测试命令是 pnpm test（vitest run）」→ 条目「测试跑 pnpm test（vitest run）」。
  - ✓（计 2）事实「本地数据库端口 5432」→ 条目「本地 PostgreSQL 映射 5432」：数字与产品名都在。
  - ✓（计 2）事实「接口超时重试 5 次，退避 200 毫秒」→ 条目「接口超时重试 3 次，退避 500 毫秒」：数值填错由 dim 1 记 0；重试与退避的主题 token 都在，「重试策略」的提问仍会命中这条。
...
When the entry is `updated: true`, the pre-session version under the same id
in `storeBefore` is the baseline: an in-place update that keeps both sides'
information is the textbook-correct merge outcome.
...
- **2 — correct new / merge / update outcome.** A genuinely new fact stands
  ...
  of an existing entry is merged into it (or updated in place) and the result
  carries both sides' information.
  - ✓（计 2）全新事实、与 `storeBefore` / `siblings` 无重叠：独立成条。
  - ✓（计 2）`updated` 条目把新事实并入既有 id，合并后新旧信息都在。
...
For one scenario, let S be every entry the session wrote or updated (written
∪ updated, per the medium-diff basis). An entry is traceable when `plantedId`
is not null — including an `updated` entry whose post-session text states a
planted fact (updating is the merge outcome, not a hallucination). Then:
...
Because the v2 basis counts in-place updates in S, v2 precision is not
comparable with v1 precision.
