...
# Recall rubric v1 — scoring standing injection and answer correctness
...
- `requiredFacts` — canonical statements of the facts the question needs
  ...
  turn whose `planted` list carries the fact id; for a seed scenario, the
  `seedEntries[].content` of the entry with that id.
...
- **1 — present but misleading, stale, or an unannotated conflict.**
  ...
  wrong: an outdated value, a statement a newer correction contradicts
  without the conflict annotation, a wrong scope/category label that routes
  search badly, or — for negative questions — an entry that reads as if the
  never-stated fact were recorded.
