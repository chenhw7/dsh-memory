...
One kind of design doc lives here. An **Agent Note** records a decision or proposal that affects this codebase — the *why* and *what we gave up*, the parts code and docs can't carry. This tree replicates the [DeepSeek Harness Agent Notes](../../../deepseek-harness/.agents/notes/README.md) convention; this file defines where Agent Notes live, when to write one, and [the in-file format](#the-file-format).

## Adaptations in this repository

The harness enforces this convention with gates — the classification gate ([scripts/agent-note-tree.ts](../../../deepseek-harness/scripts/agent-note-tree.ts)), `verify-agent-note-format` (part of its `doc-sync`), translation pairing, and [`verify-archived-agent-notes`](../../../deepseek-harness/scripts/verify-archived-agent-notes.ts) — none of which run here. Their discipline is applied by hand, and the canonical definitions are referenced read-only from the harness repository. Two mechanical deltas follow: the archived tree keeps no `manifest.json` (that file is owned by the upstream verifier), and sidecar hashes are re-recorded with `git hash-object` instead of the pairing script.
...
- **Class** (the nested folder) is the Agent Note's kind — see [Classification](#classification) below.

The date in the filename is when the topic was **first proposed** (per git history). Cross-references between Agent Notes in this tree use relative markdown links (`[topic](../../implemented/architecture/2026-…-….md)`) — never bare prose or numbers — so they are mechanically checkable and survive moves between folders. References into the upstream harness repository use plain harness paths.

The active lifecycle tree is the working inventory: browse its lifecycle/class folders or search the repository. Do not add a centralized `INDEX.md`; the [no-index Agent Note](../../../deepseek-harness/.agents/notes/implemented/process/2026-07-19-remove-generated-agent-note-index.md) owns the rationale. Low-future-value implemented records move to the separate frozen [`archived/`](archived/AGENTS.md) tree described below.

<a id="classification"></a>

## Classification

Each Agent Note belongs to one path-encoded class from the closed set in the upstream [scripts/agent-note-tree.ts](../../../deepseek-harness/scripts/agent-note-tree.ts); the upstream classification gate rejects other folders, and this repository adopts the same closed set by convention. Adding a class requires updating the canonical set and this section. See the [classification Agent Note](../../../deepseek-harness/.agents/notes/implemented/process/2026-06-20-agent-note-classification.md).
...
Every active Agent Note follows one in-file format, enforced upstream by `pnpm run verify-agent-note-format` ([scripts/verify-agent-note-format.ts](../../../deepseek-harness/scripts/verify-agent-note-format.ts), part of `doc-sync`); the rationale for the format — and the alternatives it rejected — is [the uniform-format Agent Note](../../../deepseek-harness/.agents/notes/implemented/process/2026-07-05-uniform-agent-note-format.md). Here the format is convention. Archived notes retain the format they had when sealed plus the archive-date line above.
...
`## Decision` describes shipped reality in the present tense, and the whole file is kept current with it per [implemented/AGENTS.md](implemented/AGENTS.md). `## Consequences` records what the trade-off cost **and** bought. Proposal-era headings are spec-speak here and the upstream gate rejects them: `## Proposal`, `## Plan`, `## Migration plan`, and `## Acceptance criteria` may not appear in an implemented Agent Note (the upstream [slop checklist](../../../deepseek-harness/docs/AGENTS.md) names why). A `## Testing`, `## Deferred`, or `## Related` section is fine where it states present-tense fact.
...
Alternatives are recorded, never invented. An Agent Note dated before 2026-07-05 whose alternatives are not reconstructible from the record carries this exact comment in place of the section, which the upstream gate accepts for pre-format files only:
...
A `.zh.md` counterpart mirrors its English sibling's structure section-for-section under the upstream [i18n contract](../../../deepseek-harness/docs/i18n/README.md); the machine-checked header tokens (`# Agent Note: ` and the `Status:` line) stay in English verbatim. The pairing is maintained by hand here — the upstream format gate skips `.zh.md` files, and its pairing gate does not run in this repository.
