...
- Scheduling: reuse the curator's per-N-sessions gating and ledger (the existing `curatorEveryNSessions` family in `src/review/index.ts`), once on startup + every N sessions; selection = accessCount DESC, `COALESCE(lastRecalledAt, updatedAt)` DESC, decay-window filter, top-N cap; the selection is bucketed and fed to the same consolidation call as Step 1.3; `lastRunAt`/cooldown persist via Step 1.1's meta table.
...
- Tests: a fixture proves this tier merges a reworded-duplicate pair sharing **zero** anchors (the fake-LLM answers by bucket content); cooldown does not re-fire; `sweepEnabled=false` is fully silent; meta-table lastRun survives a reopen.
- Eval acceptance (mechanical layer): replay the prog101/112/201/303 corpus (mock routing + deterministic assertions): stored entries 52 → ≈40, duplicate fact pairs 10 → 0, unannotated contradictions → 0, prog112 projectName in place; the 9/2 report's 10 duplicate-pair scenarios trigger 10/10. Counting uses the existing `readStoredEntries` (`eval/harness/seed-media.ts:98-121`) and the runner's written/updated accounting; the duplicate-pair counter is added to `eval/mechanical.ts`. The v2 judged A/B (env-gated real model) remains the behavioral gate.
