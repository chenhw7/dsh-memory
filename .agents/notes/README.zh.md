...
这里存放一类设计文档。**Agent Note** 记录影响本代码库的决策或提案：代码和文档无法承载的*为什么*以及*放弃了什么*。本目录树复刻 [DeepSeek Harness Agent Notes](../../../deepseek-harness/.agents/notes/README.zh.md) 的约定；本文件规定 Agent Note 存放在哪里、何时需要写一份，以及[文件内格式](#the-file-format)。

## 本仓库的适配

harness 侧用一组门禁执行这套约定——分类门禁（[scripts/agent-note-tree.ts](../../../deepseek-harness/scripts/agent-note-tree.ts)）、`verify-agent-note-format`（其 `doc-sync` 的一环）、翻译配对、以及 [`verify-archived-agent-notes`](../../../deepseek-harness/scripts/verify-archived-agent-notes.ts)——它们都不在本仓库运行。纪律靠人工执行，规范定义以 harness 仓库为权威、只读引用。由此带来两处机械差异：归档树不维护 `manifest.json`（该文件归上游验证器所有），sidecar hash 用 `git hash-object` 重录而非配对脚本。
...
文件名中的日期是该主题**首次提出**的时间（以 git 历史为准）。本目录树内 Agent Note 之间的交叉引用使用相对 Markdown 链接（`[topic](../../implemented/architecture/2026-…-….md)`），从不使用纯文字或编号，这样既可机械检查，也能在文件夹间移动时保持有效。指向 harness 上游仓库的引用使用纯路径。

活跃生命周期目录树就是工作清单：浏览其生命周期/类别文件夹，或搜索仓库即可。请勿添加集中式 `INDEX.md`；设计理由见上游的[不设索引 Agent Note](../../../deepseek-harness/.agents/notes/implemented/process/2026-07-19-remove-generated-agent-note-index.zh.md)。未来指导价值较低的已实施记录会移至下文所述、单独冻结的 [`archived/`](archived/AGENTS.md) 目录树。

<a id="classification"></a>

## 分类

每份 Agent Note 属于上游 [scripts/agent-note-tree.ts](../../../deepseek-harness/scripts/agent-note-tree.ts) 中封闭集合里的一个路径编码类别；上游的分类门禁拒绝其他文件夹，本仓库按约定采纳同一封闭集合。新增类别需要同时更新规范集合与本节。见上游的[分类 Agent Note](../../../deepseek-harness/.agents/notes/implemented/process/2026-06-20-agent-note-classification.zh.md)。
...
封存后，每组归档文件都永久冻结。禁止编辑、翻译、重新格式化、更新、移动或删除，也不得将其视为当前行为的权威依据。当活跃文档有意引用历史时，仍可链接到归档 Agent Note。上游的 [`verify-archived-agent-notes`](../../../deepseek-harness/scripts/verify-archived-agent-notes.ts) 在上游强制执行封闭的类别目录树、完整的三文件配对、归档元数据、伴随记录 hash，以及仅追加的冻结内容 manifest；本仓库人工维持同样的规则，且不维护 manifest 文件。[归档政策 Agent Note](../../../deepseek-harness/.agents/notes/implemented/process/2026-07-26-frozen-agent-note-archive.zh.md) 记录了设计依据。

<a id="when-to-write-one"></a>

## 何时需要写一份

每个非平凡变更都必须在同一提交中新增或更新至少一份 Agent Note。如果变更修改了行为、架构、跨文件或跨包约定、流程或工具、测试策略、磁盘存储格式、协议格式（wire format）或配置格式，或者维护者可能合理重新审视的其他决策，就属于非平凡变更。对未来重大工作的提案从 `proposed/` 开始；已经做出的决策从 `implemented/` 开始。选择与决策匹配的类别文件夹（见[分类](#classification)）。
...
每份活跃 Agent Note 遵循统一的文件内格式，在上游由 `pnpm run verify-agent-note-format`（[scripts/verify-agent-note-format.ts](../../../deepseek-harness/scripts/verify-agent-note-format.ts)，`doc-sync` 的一环）强制执行；该格式的设计动机及其否决的替代方案见上游的[统一格式 Agent Note](../../../deepseek-harness/.agents/notes/implemented/process/2026-07-05-uniform-agent-note-format.zh.md)。在本仓库，格式即约定。归档记录保留封存时的格式，并增加上述归档日期行。
...
后跟一个空行。`Status:` 的值有三种形式，且必须与文件所在的生命周期文件夹一致：
...
`## Decision` 以现在时态描述已交付的现实，整个文件按 [implemented/AGENTS.md](implemented/AGENTS.md) 的要求与之保持同步。`## Consequences` 记录权衡的代价**与**收益。提案阶段的标题在此属于规格用语，上游门禁会拒绝它们：`## Proposal`、`## Plan`、`## Migration plan` 和 `## Acceptance criteria` 不得出现在 implemented Agent Note 中（原因见上游的 [slop 检查清单](../../../deepseek-harness/docs/AGENTS.md)）。`## Testing`、`## Deferred` 或 `## Related` 章节在陈述现在时态的事实时是允许的。
...
替代方案是记录下来的，不是凭空编造的。日期早于 2026-07-05 且替代方案无法从记录中重建的 Agent Note，用以下精确注释代替该章节，上游门禁仅对格式规范之前的文件接受此注释：
...
将文件在生命周期文件夹之间移动意味着在同一个变更中更新 `Status:` 行并满足目标文件夹的骨架要求。具体而言，`proposed/` → `implemented/` 将 `## Proposal` 改写为现在时态的 `## Decision`，将 `## Acceptance criteria` 和 `## Risks` 折入 `## Consequences`（或折入一个现在时态的 `## Testing`/`## Verification` 章节，用于描述现在锁定该行为的内容），并用实际交付的内容替换计划——也就是将 [implemented/AGENTS.md](implemented/AGENTS.md) 所要求的改写变成可机械检查的规则。`proposed/` → `rejected/` 仅在 `Status:` 行添加原因并冻结文件。

### 中文对侧文件

`.zh.md` 对侧文件按上游的 [i18n 约定](../../../deepseek-harness/docs/i18n/README.zh.md)逐章节与其英文对侧文件保持相同结构；机器检查的头部标记（`# Agent Note: ` 和 `Status:` 行）保持英文原样不翻译。本仓库以人工方式维护配对——上游格式门禁跳过 `.zh.md` 文件，其配对门禁不在本仓库运行。
