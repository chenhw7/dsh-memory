...
- `npm run build`: `tsc` over `src/` plus the esbuild client bundle — this is the typecheck lane, the compile gate, and the client-bundle gate. `src/client` is type-checked by a second dedicated step in the same script, `tsc -p tsconfig.client.json` (see [the client type gate note](2026-09-01-client-typecheck-gate.md)); there is no separate `typecheck`, `lint`, or `coverage` script.
