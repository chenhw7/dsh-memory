...
3. **`real` mode mirrors the deployment's `llm-pi-ai:` section into the throwaway home** when the resolved provider is non-DeepSeek (`eval/model-route.ts` resolves it; `profile-template.ts` renders it as the throwaway settings document). The section is schema-validated on load by the adapter itself; profiles must keep credentials behind `apiKeyEnv` references — a profile carrying an inline credential field fails loud before anything is mirrored (the throwaway home and kept homes are not a credential sink). Keys resolve per request through the harness credentials seam over the inherited environment (`FUYAO_API_KEY` in the eval process env flows to the child).
...
- A deployment whose `agent-default-model` names a provider with no stored/`apiKeyEnv` credential fails at the first turn with the adapter's `MISSING_CREDENTIAL` (loud, naming the env ref and both remedies) rather than at eval parse time — the eval does not pre-read the credentials plane.
