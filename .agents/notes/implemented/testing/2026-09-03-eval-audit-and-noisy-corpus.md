...
One v2 done-ness item remains open: the core-v0 judged A/B (real model + judge, env-gated) — the first attempt on 2026-09-03 aborted half-way through the baseline pass on gateway congestion (three 120s notification timeouts, three turn-budget breaches, ~6 of 13 scenarios failed, no report produced); it is to be re-run once the gateway recovers. A 2026-09-07 real judged slice (7 scenarios, gateway healthy end to end with the transport-retry fix) produced the first readings of the standing v2 baseline and surfaced two corpus findings that gate two core scenarios — counterfactual premises falsifiable from the unsandboxed host filesystem, and non-terminating tool loops in planting dialogues that instruct repo work; both are recorded with the evidence in the [write-path rework note](../architecture/2026-09-04-write-path-rework-implementation-plan.md). The corpus answer to both findings shipped the same day: the eight repo-work plant rows now pin a workspace fixture that the runner materializes into the throwaway home and mounts as the child session's cwd ([eval workspace fixture repo](2026-09-07-eval-workspace-fixture-repo.md)); the premises are consistent in-repo and the exploration has a bounded target, so the blocked pair's rerun rides the pending judged baseline. The full core-v0 judged A/B itself remains to be run. All other acceptance evidence is above.
