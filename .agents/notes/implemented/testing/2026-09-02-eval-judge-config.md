...
1. **`$DSH_HOME/eval.yaml` (else `~/.dsh/eval.yaml`) is the eval's instrument config** (`eval/eval-config.ts`) — judge only, deliberately: the judge must never default to the model under test (same-source self-grading bias), so the deployment's settings.yaml stays the tested-model source and this file never overrides it. `judge:` carries `baseURL` + `apiKey` (pasted) or `apiKeyEnv` (resolved from the eval process environment) + `model`, optional `reasoningEffort`. A present-but-incomplete section (half-pasted key, unknown fields, both credential forms) fails loud — a half-pasted instrument must not silently degrade to skipped judging.
