...
- **治理面**：独立 Identity 设置区（id `identity`、order 26）：文档只读渲染 + 版本历史 + 两步 revert（`identityRevertEnabled` 默认**开**，独立于 `remoteWritesEnabled`——恢复面被历史里过安检的旧文本界定，并入默认关的开关会让只读约束下的人类没有任何阀门）+ 导出（无导入）。remote 读方法不设门，写方法仅 revert 一个。
