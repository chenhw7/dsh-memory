...
Distilled from the upstream harness repository's 2026-08 repo-wide purge and its review rounds; this repository adopts them as its calibration standard. Use them to identify the governing principle, not as text templates. This file deliberately quotes leaked wording as calibration material — the [recall batteries](recall-batteries.md) exclude the skill's directory, and its wording is not a license elsewhere.
...
**Fixed:** "Slash input resolves against the visible catalog — the plain-text-reference decision, owned by the web input-machine note in the upstream harness repository (that note lives in the host repo; this example set was distilled there and is kept as calibration only)."

The ordinal resolves nowhere at HEAD; the decision's name and owning note path do. This example cites an upstream harness note because the example itself came from the harness purge — in this repository, cite this repository's own notes the same way (for example, [the committed-artifact-citations note](../../../notes/implemented/process/2026-08-31-committed-artifact-citations.md)). Name the owning note's path at least once per file — as a link where the surface supports one — and later mentions may use the searchable name alone.
