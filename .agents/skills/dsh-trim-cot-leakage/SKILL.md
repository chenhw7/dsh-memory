...
1. Scope and exclusions per [dsh-prose-standard](../dsh-prose-standard/SKILL.md): require an explicit scope; never touch `lib/`, `node_modules/`, `docs/archive/`, or `.agents/notes/archived/`. Test fixtures and golden corpora are derivatives, not prose targets: change the owning source or scenario and regenerate them only when an authorized behavior change requires new evidence.
...
3. Fix owner-first per surface: extracted prompt sections and README tables → trace every consumer, fix the source JSDoc or owning prose, then regenerate derivatives; bilingual prose → update the counterpart minimally and re-record it through the [lightweight routine](../../../docs/AGENTS.md#writing-rules); bilingual fences → copy the corrected verbatim block byte-for-byte into both sides per [dsh-doc](../dsh-doc/SKILL.md), then re-record the sidecar; model- or user-visible strings → route through [dsh-prose-standard](../dsh-prose-standard/SKILL.md) and change only with owning behavior evidence, otherwise leave unchanged and report the deferral.
...
5. Verify: re-run the batteries expecting only sanctioned keeps, this skill's own directory, and the owning note's quoted evidence; confirm every remaining citation resolves at HEAD; then check the touched surfaces by hand — relative links resolve, paired sides say the same thing with re-recorded sidecars ([i18n contract](../../../docs/i18n/README.md)), and `npm run build` still passes when JSDoc or types were touched.
