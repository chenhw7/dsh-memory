...
description: Use when writing, reviewing, restoring, trimming, or auditing prose in this repository (dsh-memory), including deciding where documentation or comments are required across Markdown, JSDoc, code and test comments, prompts, descriptions, diagnostics, and CLI or UI strings.
---
...
Always exclude `node_modules/`, `lib/` (built output), and `docs/archive/` from discovery, review, and edits, even when the requested scope is the whole repository. Do not follow a symlink into an ignored tree. Put exclusions after inclusion globs so a later include cannot re-admit it: for example, end ripgrep commands with `--glob '!lib/**' --glob '!node_modules/**'`, and give Git commands an explicit `:(exclude)lib/**` pathspec. If the requested scope contains only excluded trees, report that no eligible files remain.
...
Treat test fixtures, golden corpora, and the esbuild client bundle as derivative. Trace every consumer before editing: source JSDoc and config descriptions may also feed the settings UI and README tables. Edit the owning source or scenario first, then regenerate every derivative. Bilingual pairs have no permanent owner: either language may be the authored side for an update. Follow the [lightweight routine path](../../../docs/AGENTS.md#writing-rules), update the counterpart minimally, and re-record the sidecar.
...
- **READMEs:** include the consumer contract: install, configuration, semantics, failures, limitations, and model-visible effects. Quote stable model-visible text owned by the package; link [TECH_DESIGN](../../../docs/TECH_DESIGN.md) for subsystem detail and cross-package owners. Keep durable gaps and maintainer traps, not ordinary cleanup inventories. Follow [docs/AGENTS.md](../../../docs/AGENTS.md) tier rules.
...
- **Prompts and visible strings:** treat wording as behavior. Client settings-card copy and rendered prompt sections are model- or user-visible behavior; inspect text, accessibility names, tooltips, placeholders, and format templates together. A wording change that reaches the model (system-prompt sections, tool descriptions) changes behavior evidence too — update the owning test in the same change. If the authorized scope has no owning test, leave the wording unchanged and report the deferral; do not silently fold it into a prose-only edit.
...
6. Run the narrow relevant checks (`npm run build` for type surfaces, focused vitest files for behavior), `git diff --check`, and behavior tests for visible strings. Verify the final diff contains no `lib/` or `node_modules/` path and report any accidental match rather than claiming a clean exclusion history.
