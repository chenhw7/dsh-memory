...
description: Use when landing a stack of dependent GitHub PRs (A ← B ← C, where each bases on the one below) onto the trunk, merging a PR whose base is another open PR's branch, or whenever a request mentions "stacked PRs", "PR stack", "dependent PRs", or merging several related PRs in sequence. Requires every same-repository dependency chain to use GitHub's official stacked-PR feature before landing so GitHub owns stack-wide rules, CI, ordering, retargeting, and merge state.
---

# Landing an official GitHub PR stack

Land dependent PRs through GitHub's native stack object and `gh stack merge`. Do not reproduce stack semantics by merging and retargeting individual PRs with `gh pr merge` and `gh pr edit`. The host repository's [AGENTS.md](../../../../deepseek-harness/AGENTS.md) owns the allowed merge-forward and rebase histories for harness work; the [stack review guide](../../../../deepseek-harness/docs/cookbook/responding-to-pr-review-on-a-stack.md) owns review-fix propagation.

**Scope note.** This repository (dsh-memory) is single-maintainer with direct pushes to `main` and no PR workflow — this skill applies only when working in the upstream harness repository or another repo that uses PRs.
