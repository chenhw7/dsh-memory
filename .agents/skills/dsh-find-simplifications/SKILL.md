...
- Treat recorded intentional designs as intentional by default: the zero-dependency goal behind the hand-rolled BM25 kernel and CJK tokenizer ([TECH_DESIGN](../../../docs/TECH_DESIGN.md)), the policy-only injection default ([INDEX_MODE_EVALUATION](../../../docs/INDEX_MODE_EVALUATION.zh.md)), and the notesDir containment rules are settled decisions. Do not propose removing them as "low effort" unless the user explicitly overrides that constraint.
