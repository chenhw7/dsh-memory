...
- **Runner infrastructure:** checkout, dependency download, disk, host process, or runner service fails independently of the test command. Require direct runner evidence before assigning this class. The release workflow runs on GitHub-hosted ubuntu runners, so this class is rare here; when the same signature repeats across unrelated tags on the runner, that is evidence of shared-host contention even when the host cannot be inspected.
...
4. a concurrent `npm run build` (or client esbuild bundle) beside the vitest run, matching what the developer host actually runs together;
5. the release workflow job — reachable only through CI, so use it as the reproduction and change one suspected owner per run.
