...
5. No verifier runs in this repository — the archive is maintained by hand, per [the notes-tree adaptation](../../notes/README.md#adaptations-in-this-repository). In place of the upstream verifier: prove every previously archived triplet still satisfies the frozen-triplet rules, add the new `Archived:` lines and sidecar re-record mechanically, and re-check the complete triplet afterward.
...
No archive verifier exists in this repository; validate by inspection instead — frozen-triplet rules on every previously archived note and the new one, sidecar hashes re-recorded, and inbound links from active prose repaired. Run `npm run test` only when archival touched prose outside `.agents/notes/`, and `git diff --check` always; select any additional evidence through [dsh-pre-push-checks](../dsh-pre-push-checks/SKILL.md).

Report active implemented notes kept, implemented notes archived, rejected notes kept/deleted, proposed notes rejected if any, and every genuinely borderline case with its word count and chosen outcome. This skill's examples quote upstream (harness) notes as calibration; classify this repository's notes against its own corpus, not those examples.
