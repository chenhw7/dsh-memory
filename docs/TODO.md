...
- **Verification caveat (✅ verified against `~/deepseek-harness`):** the client-module manifest
  field (`dsh.client`), the `/plugins/<id>/client.js` serving path, the `@Remote` gateway, and the
  `settings.section` slot are all **confirmed against the pinned host checkout** —
  `docs/subsystems/client-modules.md` (manifest + serving), `docs/api-gateway.md` (`@Remote` +
  `TypertRemoteService` + `ctx.remote.*`), and `packages/client/ui-settings/src/client/contract/slots.ts`
  (`settings.section` slot, `kind: 'list'`, `scope: 'root'`). Real precedents confirmed:
  `dsh-message-feedback` (`MessageFeedbackService` — CRUD over a sidecar table, closest analog),
  `dsh-goal` (`GoalService` — live-Agent `@Remote`), `ui-agent-preset` (settings section registration
  with arbitrary React TSX via `ctx.slots.inject('settings.section', ...)`).

  **Constraints discovered (not in the original plan):**
  1. **Host modification required:** the `@Remote` service's client face must be mounted in
     `packages/api/remotes/src/client/index.ts` (import + `ctx.remote.$mount()` + peerDependency).
     This is a host-code change, conflicting with §6's no-host-change policy. §3.8 is therefore
     blocked until the host team accepts a PR to `dsh-api-remotes`, or the policy is relaxed.
  2. **`dsh.client` lives on the assembly package** (`@deepseek-ai/dsh-api-remotes`), not on the
     memory plugin package itself. The plugin only needs `exports["./typert"]` and
     `exports["./remote"]` subpaths.
  3. **Typert codegen:** `lib/typert.host.js` and `lib/typert.remote-client.js` are generated by
     the host build pipeline (`tsdown` + `typertPlugin`), not hand-written. The plugin must be in
     the host workspace for the generator to discover it.
  4. **Write methods should be pinned to loopback** via `PRIVILEGED_METHODS` in
     `packages/client/connection/src/index.ts` — `trustedHosts` is a DNS-rebinding fence, not
     authentication; without pinning, LAN callers could reach mutating memory methods.
  5. **UI slot registration:** `ctx.slots.inject('settings.section', () => ctx.slots.register({
     name: 'settings.section', id: 'memory', order, label, locale, inject }, Component))` —
     the component is arbitrary React TSX receiving `{ close }` + framework hooks.
- **Done when:** full CRUD round-trip from the UI through `ctx.remote.memory.*`; changes are
  ...
  writes; the `@Remote` service is mounted in `dsh-api-remotes` and write methods are pinned to
  loopback via `PRIVILEGED_METHODS`.
