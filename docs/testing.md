...
- **Build gate** (`npm run build`): `tsc` over `src/` plus the esbuild client bundle. TypeScript is the typecheck lane; there is no separate coverage, lint, or snapshot lane. GitHub Actions runs this same `build` + `test` pair twice: `ci.yml` on every push to `main` and every pull request, and again through `prepublishOnly` before `npm publish` on a version tag — a failing suite blocks both the CI signal and the release ([release pipeline](TECH_DESIGN.md)).
