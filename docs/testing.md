...
- **Golden corpora**: `tests/fixtures/` holds checked-in datasets — the dedup dataset, extraction goldens, and the scanner corpus. `tests/recall-golden.spec.ts` pins the injection-mode comparison measured in [INDEX_MODE_EVALUATION.zh.md](INDEX_MODE_EVALUATION.zh.md); a change that intentionally shifts a golden number updates the fixture and the evaluation document in the same change.
