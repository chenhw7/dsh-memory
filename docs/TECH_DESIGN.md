...
- **Build:** `npm run build` runs two tsc programs in sequence — the host `tsc -p tsconfig.json` (`src/`, excluding `src/client`) and the client type gate `tsc -p tsconfig.client.json` (`src/client`, `noEmit`, JSX/DOM aligned to the browser runtime, with the host client packages whose types this package does not install mapped via `paths` to the harness workspace's type artifacts) — then `scripts/fix-imports.cjs` fixes `.ts → .js` specifiers in tsc output and copies Typert artifacts, and finally `scripts/build-client.cjs` esbuild-bundles the TSX client into a loader-compatible IIFE with host packages external (resolved at runtime through the injected `require`). A client type error therefore fails the build exactly like a host one.
...
| Another host on the network reading or writing the store | Memory RPC carries no per-method authorization (§7.7): the host's transport trust fence is the only gate. Any host admitted through the host-owned `trustedHosts` gets full read/write over memory, and anything written reaches later sessions' system prompts — a persistent injection channel. Keep the fence at loopback unless every admitted host is trusted |
...
The repo ships **28 vitest spec files, 530 test cases** (524 active + 6 skipped without real-API keys), in five layers:
