...
- **Nine memory tools + the identity tool** — `memory_search`, `memory_add`, `memory_replace`, `memory_remove`, `memory_list`, `memory_get`, `memory_pin`, `memory_unpin`, `memory_forget`, plus `identity_update` (below).
- **Identity layer (opt-in)** — the agent's two self-documents, **SOUL.md** (its character) and **USER.md** (its understanding of you), grown *by the agent in conversation* through `identity_update` and injected into every session (`soul` @80 / `user-profile` @81, coexisting with — and yielding to — the deployment persona; explicit conversation instructions always win). Seeded once from built-in Chinese copy, versioned with full history, never decayed. You hold no editor — only governance: a read-only Identity settings section with version history, a two-step **revert** valve (default on, separate from the remote write switch), and markdown export. Extraction never persists identity restatements (prompt rule + mechanical prefilter), and an eval slice + `--identity` axis pin the injection surface (see [TECH_DESIGN §7.10](docs/TECH_DESIGN.md)).
